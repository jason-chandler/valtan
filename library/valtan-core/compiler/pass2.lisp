...
(defparameter *builtin-function-table*
  (let ((table (make-hash-table)))
    (setf (gethash (read-from-string "COMMON-LISP:SYMBOLP") table) (list "lisp.CL_symbolp" (list 1)))
    (setf (gethash (read-from-string "SYSTEM::MAKE-SYMBOL") table)
          (list "lisp.CL_makeSymbol" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:SYMBOL-PLIST") table)
    ...
    (setf (gethash (read-from-string "COMMON-LISP:BOUNDP") table) (list "lisp.CL_boundp" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:FBOUNDP") table) (list "lisp.CL_fboundp" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:SYMBOL-VALUE") table)
          (list "lisp.CL_symbolValue" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:SYMBOL-FUNCTION") table)
          (list "lisp.CL_symbolFunction" (list 1)))
    (setf (gethash (read-from-string "SYSTEM:%SET") table) (list "lisp.CL_set" (list 2)))
    (setf (gethash (read-from-string "COMMON-LISP:MAKUNBOUND") table) (list "lisp.CL_makunbound" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:FMAKUNBOUND") table) (list "lisp.CL_fmakunbound" (list 1)))
    ...
    (setf (gethash (read-from-string "COMMON-LISP:PACKAGEP") table) (list "lisp.CL_packagep" (list 1)))
    ...
    (setf (gethash (read-from-string "COMMON-LISP:LIST-ALL-PACKAGES") table)
    ...
    (setf (gethash (read-from-string "COMMON-LISP:NUMBERP") table) (list "lisp.CL_numberp" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:INTEGERP") table) (list "lisp.CL_integerp" (list 1)))
    ...
    (setf (gethash (read-from-string "COMMON-LISP:FUNCTIONP") table) (list "lisp.CL_functionp" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:CONSP") table) (list "lisp.CL_consp" (list 1)))
    (setf (gethash (read-from-string "COMMON-LISP:CONS") table) (list "lisp.CL_cons" (list 2)))
    ...
    (setf (gethash (read-from-string "COMMON-LISP:VALUES") table) (list "lisp.CL_values" (list 0 nil)))
    ...
    (setf (gethash (read-from-string "COMMON-LISP:EQ") table) (list "lisp.CL_eq" (list 2)))
    (setf (gethash (read-from-string "SYSTEM::ERROR") table) (list "lisp.CL_error" (list nil)))
    (setf (gethash (read-from-string "COMMON-LISP:CHARACTERP") table) (list "lisp.CL_characterp" (list 1)))
    ...
    (let ((err (p2-genvar)))
      (p2-emit-try-catch
       ...
       ((err)
        ...
        (format *p2-emit-stream* "console.log(~A.message);~%" err))))
    (write-line "// initialize-vars" stream)
    (p2-emit-initialize-vars stream)
    (write-line "// toplevel defun" stream)
    (join-emitter-stream stream *p2-toplevel-defun-stream*)
